"""Tests for enhanced filesystem tools: ReadFileTool, ReadImageTool, EditFileTool, ListDirTool."""
...
class TestReadImageTool:
    ...
    @pytest.mark.asyncio
    async def test_read_png_returns_json_data_url(self, tool, tmp_path):
        img = tmp_path / "x.png"
        ...
        assert data["size_bytes"] == len(_MIN_PNG)
        assert data["data_url"].startswith("data:image/png;base64,")

    @pytest.mark.asyncio
    async def test_not_image(self, tool, tmp_path):
        f = tmp_path / "a.txt"
        f.write_text("hello", encoding="utf-8")
        result = await tool.execute(path=str(f))
        assert "Error" in result
        assert "recognized image" in result

    @pytest.mark.asyncio
    async def test_too_large(self, tool, tmp_path):
        t = ReadImageTool(workspace=tmp_path)
        img = tmp_path / "big.bin"
        # Fake PNG header + padding (not valid full PNG but passes size check first... actually detect_image_mime checks magic - need PNG magic)
        img.write_bytes(b"\x89PNG\r\n\x1a\n" + b"x" * (ReadImageTool._MAX_BYTES + 1))
        result = await tool.execute(path=str(img))
